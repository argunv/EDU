...
from app.models.role_profiles import ParentStudentLink, UserRole
...
def check_teacher_schedule_conflicts(
    body: list[AdminScheduleChange], db: Session
) -> None:
    """Ensure one teacher is not in two classes at the same time (400 if so)."""
    ...
    for ch in body:
        ...
        if parsed:
            ...
            deleted_slot_keys.add(
                (cid, shift, (day_label or "").strip(), lesson_number)
            )
    ...
    for ch in body:
        ...
        updated_or_created_slot_keys.add(
            (
                cid,
                ch.slot.shift,
                (ch.slot.day_label or "").strip(),
                ch.slot.lesson_number,
            )
        )
    ...
    def slot_key(s: AdminScheduleSlotDraft) -> tuple:
        return (
            s.shift,
            (s.day_label or "").strip(),
            s.lesson_number,
            (s.teacher_id or "").strip(),
        )
    ...
    for (
        ...
        lesson_number,
        teacher_id,
    ), class_ids in assignments_by_slot.items():
        if len(class_ids) > 1:
            time_str = slot_time_by_key.get(
                (shift, day_label, lesson_number, teacher_id), "?"
            )
            raise HTTPException(
    ...
    for ch in body:
        ...
        if other:
            ...
            time_str = (
                getattr(other, "time", None) or ch.slot.time or ""
            ).strip() or "?"
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
...
def apply_schedule_changes(body: list[AdminScheduleChange], db: Session) -> None:
    ...
    cancelled_to_notify: list[tuple[UUID, str, str, str, str]] = []
    ...
    for ch in body:
        ...
        if existing:
            ...
            if not was_cancelled and new_cancelled:
                cancelled_to_notify.append(
                    (cid, cls.name, sub.name, day_label, time_str)
                )
        else:
            db.add(
                ScheduleSlot(
    ...
    for cid in classes_touched:
        ...
        if not cls:
            continue
        row = (
            db.query(func.max(ScheduleSlot.lesson_number))
            .filter(
                ScheduleSlot.class_id == cid,
            )
            .scalar()
...
def _resolve_teacher_id(
    db: Session, teacher_id_raw: str | None, teacher_name: str | None
) -> UUID:
    if teacher_id_raw:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Некорректный teacher_id",
            ) from exc
        exists = (
    ...
    if len(rows) != 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="teacher_name неоднозначен, укажите teacher_id",
        )
    return rows[0][0]
