...
from sqlalchemy import CheckConstraint, Column, String, Date, Time, ForeignKey, Text, UniqueConstraint
...
class Lesson(Base):
    ...
    __table_args__ = (
        UniqueConstraint("class_id", "subject_id", "date", "time", name="uq_lesson_slot"),
    )
...
class LessonAttendance(Base):
    ...
    __table_args__ = (
        UniqueConstraint("lesson_id", "student_id", name="uq_lesson_attendance_student"),
        CheckConstraint("attendance in ('present','absent')", name="ck_lesson_attendance_value"),
    )
